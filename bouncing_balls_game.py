...
while running:
    ...
    for ball in balls[:]:  # Χρησιμοποιούμε slice για ασφαλή διαγραφή
        ...
        if dist + BALL_RADIUS > CIRCLE_RADIUS:
            if is_ball_in_arc(ball.pos, CIRCLE_CENTER, start_angle, end_angle):
                ...
                # Αντιστοίχιση στη περιφέρεια
                d = ball.pos - CIRCLE_CENTER
                d_unit = d / np.linalg.norm(d)
                ball.pos = CIRCLE_CENTER + d_unit * (CIRCLE_RADIUS - BALL_RADIUS)  # ΔΙΟΡΘΩΣΗ: αφαίρεσα το έξτρα d_unit
                
                # Ανάκλαση
                t = np.array([-d_unit[1], d_unit[0]], dtype=np.float64)
                proj_v_t = np.dot(ball.v, t) * t
                ball.v = 2 * proj_v_t - ball.v
                
                # Προσθήκη εφαπτομενικής δύναμης
                ball.v += t * spinning_speed * 50  # Πολλαπλασίασε για καλύτερη οπτική επίδραση
